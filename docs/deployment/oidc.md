...
|----------|-------------|
| `KROKI_MCP_BASE_URL` | Public base URL of the server (e.g. `https://mcp.example.com`; include prefix when mounted under subpath, e.g. `https://mcp.example.com/myservice`) |
| `KROKI_MCP_OIDC_CONFIG_URL` | OIDC discovery endpoint (e.g. `https://auth.example.com/.well-known/openid-configuration`) |
| `KROKI_MCP_OIDC_CLIENT_ID` | OIDC client ID registered with your provider |
...
|----------|---------|-------------|
| `KROKI_MCP_OIDC_JWT_SIGNING_KEY` | ephemeral | JWT signing key. **Required on Linux/Docker** — the default is ephemeral and invalidates tokens on restart |
| `KROKI_MCP_OIDC_AUDIENCE` | — | Expected JWT audience claim; leave unset if your provider does not set one |
| `KROKI_MCP_OIDC_REQUIRED_SCOPES` | `openid` | Comma-separated required scopes |
| `KROKI_MCP_OIDC_VERIFY_ACCESS_TOKEN` | `false` | Set `true` to verify the upstream access token as JWT instead of the id token. Only needed when your provider issues JWT access tokens and you require audience-claim validation on that token |
...
!!! danger "Linux / Docker"
    On Linux (including Docker), the ephemeral key is especially problematic because it does not persist across process restarts. Always set `KROKI_MCP_OIDC_JWT_SIGNING_KEY` in production.
...
```bash
KROKI_MCP_BASE_URL=https://mcp.example.com
KROKI_MCP_OIDC_CONFIG_URL=https://auth.example.com/.well-known/openid-configuration
KROKI_MCP_OIDC_CLIENT_ID=my-kroki-mcp
KROKI_MCP_OIDC_CLIENT_SECRET=your-client-secret
KROKI_MCP_OIDC_JWT_SIGNING_KEY=$(openssl rand -hex 32)
```
...
```
Client → kroki-mcp (with OIDCProxy) → OIDC Provider (Authelia/Keycloak)
```
...
services:
  kroki-mcp:
    image: ghcr.io/pvliesdonk/kroki-mcp:latest
    env_file: .env
    volumes:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.kroki-mcp.rule=Host(`mcp.example.com`)"
      - "traefik.http.routers.kroki-mcp.tls.certresolver=letsencrypt"
      - "traefik.http.services.kroki-mcp.loadbalancer.server.port=8000"
    networks:
      - traefik
...
```bash
KROKI_MCP_READ_ONLY=true
KROKI_MCP_BASE_URL=https://mcp.example.com
KROKI_MCP_OIDC_CONFIG_URL=https://auth.example.com/.well-known/openid-configuration
KROKI_MCP_OIDC_CLIENT_ID=my-kroki-mcp
KROKI_MCP_OIDC_CLIENT_SECRET=your-client-secret
KROKI_MCP_OIDC_JWT_SIGNING_KEY=your-stable-hex-key
```
...
```bash
KROKI_MCP_BASE_URL=https://mcp.example.com/myservice
KROKI_MCP_HTTP_PATH=/mcp
```
