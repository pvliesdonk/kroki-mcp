...
|------|-------------|---------------|
| **Multi-auth** | Mixed clients — e.g. Claude web (OIDC) + Claude Code (bearer token) on the same server | Set both `KROKI_MCP_BEARER_TOKEN` and all four OIDC variables |
| **Bearer token** | Simple deployments behind a VPN, Docker compose stacks, development | Set `KROKI_MCP_BEARER_TOKEN` only |
...
2. Set the environment variable:
    ...
    ```bash
    KROKI_MCP_BEARER_TOKEN=your-generated-token
    ```
...
```
Client → kroki-mcp (OIDCProxy) → OIDC Provider
```
...
|----------|-------------|
| `KROKI_MCP_BASE_URL` | Public base URL (e.g. `https://mcp.example.com`) |
| `KROKI_MCP_OIDC_CONFIG_URL` | OIDC discovery endpoint |
| `KROKI_MCP_OIDC_CLIENT_ID` | Client ID registered with your provider |
| `KROKI_MCP_OIDC_CLIENT_SECRET` | Client secret |

### Optional variables

| Variable | Default | Description |
|----------|---------|-------------|
| `KROKI_MCP_OIDC_JWT_SIGNING_KEY` | ephemeral | JWT signing key — **required on Linux/Docker** |
...
| `KROKI_MCP_OIDC_VERIFY_ACCESS_TOKEN` | `false` | Set `true` to verify the access token as a JWT instead of the id token; useful for audience-claim validation on JWT access tokens |
...
You're missing `KROKI_MCP_OIDC_JWT_SIGNING_KEY`. Without it, FastMCP generates an ephemeral key on each startup. Generate and set a stable key:
